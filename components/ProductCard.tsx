...
export default function ProductCard({ product, onClick, selected }: ProductCardProps) {
  return (
    ...
    >
      ...
      <img src={product.imageUrl} alt={product.name} className="w-full h-40 sm:h-48 object-cover rounded-lg mb-4" />
      <div className="flex-1">
        <h3 className="text-lg sm:text-xl font-bold text-zinc-100">{product.name}</h3>
        <p className="text-xs sm:text-sm text-zinc-400 mb-2">{product.category}</p>
        <p className="text-blue-400 font-bold text-base sm:text-lg mb-4">${product.price}</p>
      </div>
